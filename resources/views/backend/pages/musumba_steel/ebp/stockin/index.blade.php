                        ...
                        <table id="dataTable" class="text-center">
                            ...
                            <tbody>
                               ...
                               <tr>
                                    ...
                                    <td><a href="{{ route('admin.stockins.show',$stockin->stockin_no)}}">{{ $stockin->stockin_no }}</a></td>
                                    ...
                                    <td>
                                        ...
                                        @if($stockin->status == 1)
                                            <a class="btn btn-primary text-white" href="{{ route('admin.stockins.validate', $stockin->stockin_no) }}"
                                            onclick="event.preventDefault(); document.getElementById('validate-form-{{ $stockin->stockin_no }}').submit();">
                                                Valider
                                            </a>

                                            <form id="validate-form-{{ $stockin->stockin_no }}" action="{{ route('admin.stockins.validate', $stockin->stockin_no) }}" method="POST" style="display: none;">
                                                @method('PUT')
                                                @csrf
                                            </form>
                                        @endif
                                        ...
                                        @if($stockin->status == 2)
                                            <a class="btn btn-primary text-white" href="{{ route('admin.stockins.confirm', $stockin->stockin_no) }}"
                                            onclick="event.preventDefault(); document.getElementById('confirm-form-{{ $stockin->stockin_no }}').submit();">
                                                Confirmer
                                            </a>

                                            <form id="confirm-form-{{ $stockin->stockin_no }}" action="{{ route('admin.stockins.confirm', $stockin->stockin_no) }}" method="POST" style="display: none;">
                                                @method('PUT')
                                                @csrf
                                            </form>
                                        @endif
                                        ...
                                        @if($stockin->status == 3)
                                            <a class="btn btn-primary text-white" href="{{ route('admin.stockins.approuve', $stockin->stockin_no) }}"
                                            ...
                                            <form id="approuve-form-{{ $stockin->stockin_no }}" action="{{ route('admin.stockins.approuve', $stockin->stockin_no) }}" method="POST" style="display: none;">
                                                @method('PUT')
                                                @csrf
                                            </form>
                                        @endif
                                        ...
                                        @if (Auth::guard('admin')->user()->can('musumba_steel_facture.reject'))
                                            ...
                                            <a class="btn btn-primary text-white" href="{{ route('admin.stockins.reject', $stockin->stockin_no) }}"
                                            onclick="event.preventDefault(); document.getElementById('reject-form-{{ $stockin->stockin_no }}').submit();">
                                                Rejeter
                                            </a>
                                            @endif
                                            <form id="reject-form-{{ $stockin->stockin_no }}" action="{{ route('admin.stockins.reject', $stockin->stockin_no) }}" method="POST" style="display: none;">
                                                @method('PUT')
                                                @csrf
                                            </form>
                                        @endif
                                        @if (Auth::guard('admin')->user()->can('musumba_steel_facture.reset'))
                                            ...
                                            <a class="btn btn-primary text-white" href="{{ route('admin.stockins.reset', $stockin->stockin_no) }}"
                                            onclick="event.preventDefault(); document.getElementById('reset-form-{{ $stockin->stockin_no }}').submit();">
                                                Annuler
                                            </a>
                                            @endif
                                            <form id="reset-form-{{ $stockin->stockin_no }}" action="{{ route('admin.stockins.reset', $stockin->stockin_no) }}" method="POST" style="display: none;">
                                                @method('PUT')
                                                @csrf
                                            </form>
                                        @endif
                                        @if (Auth::guard('admin')->user()->can('musumba_steel_facture.edit'))
                                            <a class="btn btn-success text-white" href="{{ route('admin.stockins.edit', $stockin->stockin_no) }}">@lang('messages.edit')</a>
                                        @endif

                                        @if (Auth::guard('admin')->user()->can('musumba_steel_facture.delete'))
                                            @if($stockin->status == -1 || $stockin->status == 1)
                                            <a class="btn btn-danger text-white" href="{{ route('admin.stockins.destroy', $stockin->stockin_no) }}"
                                            onclick="event.preventDefault(); document.getElementById('delete-form-{{ $stockin->stockin_no }}').submit();">
                                                @lang('messages.delete')
                                            </a>

                                            <form id="delete-form-{{ $stockin->stockin_no }}" action="{{ route('admin.stockins.destroy', $stockin->stockin_no) }}" method="POST" style="display: none;">
                                                @method('DELETE')
                                                @csrf
                                            </form>
                                            @endif
                                        ...
                                        <a href="{{ route('admin.stockins.bonEntree',$stockin->stockin_no) }}"><img src="{{ asset('img/ISSh.gif') }}" width="60" title="Télécharger d'abord le document et puis imprimer"></a>
